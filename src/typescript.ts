 ...
 */
export function getSourceFiles(rootFileNames: string[], options: ts.CompilerOptions): string[] {
    ...
    const programmFiles = program
        ...
        .filter(file => {
            ...
            return arr.indexOf('node_modules') < 0 && arr[arr.length - 1].indexOf('.d.ts') < 0
        })
    return programmFiles
}

export function getTypescriptConfig(cwd: string, logger?: { log: (str: string) => void }): ts.ParsedCommandLine {
